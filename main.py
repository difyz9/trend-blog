...
try:
    ...
    print(f"⚠️ Markdown/GitHub功能不可用: {e}")
...
def load_config():
    ...
    if config.get("GITHUB", {}).get("enabled", False):
        ...
        if repo_url:
            ...
            print(f"GitHub推送已启用: {safe_url} (分支: {config['GITHUB']['branch']})")
        else:
            print("⚠️ GitHub推送已启用但未配置repo_url")
...
def get_output_path(subfolder: str, filename: str) -> str:
    ...
    # 根据子文件夹类型，检查是否有专门的配置
    if subfolder == "txt" and CONFIG.get("OUTPUT_TXT_DIR"):
        output_dir = Path(CONFIG["OUTPUT_TXT_DIR"])
    elif subfolder == "html" and CONFIG.get("OUTPUT_HTML_DIR"):
        output_dir = Path(CONFIG["OUTPUT_HTML_DIR"])
    else:
        # 默认在基础目录下按日期创建子目录
        output_dir = Path(base_dir) / date_folder / subfolder
...
def is_first_crawl_today() -> bool:
    ...
    # 如果配置了专门的txt目录，使用它；否则使用默认的日期子目录
    if CONFIG.get("OUTPUT_TXT_DIR"):
        txt_dir = Path(CONFIG["OUTPUT_TXT_DIR"])
    else:
        txt_dir = Path(base_dir) / date_folder / "txt"
...
class PushRecordManager:
    ...
    def __init__(self):
        push_records_dir = CONFIG.get("OUTPUT_PUSH_RECORDS_DIR", "output/.push_records")
        self.record_dir = Path(push_records_dir)
