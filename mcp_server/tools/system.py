        ...
        try:
            ...
            if config_path.exists():
                with open(config_path, "r", encoding="utf-8") as f:
                    ...
                    self.base_dir = output_config.get("base_dir", "output")
                    self.txt_dir_override = output_config.get("txt_dir", "")
                    self.html_dir_override = output_config.get("html_dir", "")
            else:
                self.base_dir = "output"
                self.txt_dir_override = ""
                self.html_dir_override = ""
        except Exception as e:
            print(f"加载输出目录配置失败: {e}，使用默认值")
            self.base_dir = "output"
            self.txt_dir_override = ""
            self.html_dir_override = ""
        ...
        try:
            ...
            if save_to_local:
                try:
                    ...
                    # 创建 txt 文件路径（使用配置的目录）
                    if self.txt_dir_override:
                        txt_dir = self.project_root / self.txt_dir_override
                    else:
                        txt_dir = self.project_root / self.base_dir / date_folder / "txt"
                    ensure_directory_exists(str(txt_dir))
                    txt_file_path = txt_dir / f"{time_filename}.txt"

                    # 创建 html 文件路径（使用配置的目录）
                    if self.html_dir_override:
                        html_dir = self.project_root / self.html_dir_override
                    else:
                        html_dir = self.project_root / self.base_dir / date_folder / "html"
