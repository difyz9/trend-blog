...
>>>>>>> 4b731d51dfe9bd9ccefddb0bb956ddd68bdfc623
...
### **GitHub 自动推送（v3.0.6 新增）** 🎉

通过 GitHub Actions 实现新闻内容自动推送到个人仓库，打造专属新闻归档站

- **自动化推送**：每次抓取新闻后自动推送 Markdown 格式到指定仓库
- **GitHub Pages展示**：配合 GitHub Pages 实现网页版新闻归档
- **零成本部署**：完全依托 GitHub 基础设施，无需额外服务器
- **历史归档**：自动保存所有历史新闻，随时查看过往热点
- **多端访问**：通过 GitHub Pages 链接在任何设备访问新闻内容

<details>
<summary><strong>👉 快速配置 GitHub 推送</strong></summary>
<br>
...
2. 生成新 Token (classic)，勾选 `repo` 权限
3. 在 TrendRadar 仓库中添加 Secret：
...
在目标仓库中：Settings → Pages → Source 选择 `main` 分支和 `/docs` 目录

**详细配置指南**：参考 [GITHUB_ACTIONS_SETUP.md](GITHUB_ACTIONS_SETUP.md)

**推送效果**：
- 自动推送 Markdown 文件到 `docs/news/` 目录
- 通过 `https://你的用户名.github.io/仓库名/news/` 访问
- 每次运行自动更新，保持内容最新
...
- **优化输出目录结构**：Markdown文件现在直接保存到日期文件夹下（如 `docs/2025年11月17日/`），不再使用单独的markdown子文件夹
- **简化配置**：移除 `markdown_dir` 配置项，markdown文件与html、txt文件处于同一目录级别
- **目录结构**：
  ```
  docs/
  ├── 2025年11月17日/
  │   ├── 热点新闻_20251117_0721.md  ← markdown文件直接在日期文件夹下
  │   ├── html/
  │   │   └── 当日汇总.html
  │   └── txt/
  │       └── 07时21分.txt
...
3. **配置说明：**:
    ...
    **输出目录结构说明**（v3.0.6+）：
    - 所有输出文件（HTML、TXT、Markdown）现在都保存在统一的日期文件夹下
    - 目录结构示例：
      ```
      docs/
      ├── 2025年11月17日/
      ...
      └── .push_records/
      ```
    - Markdown文件与html、txt文件夹处于同一级别，方便统一管理和推送到GitHub
